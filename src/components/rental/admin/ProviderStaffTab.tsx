...
export function ProviderStaffTab({ provider }: Props) {
  ...
  const { data: owner } = useQuery({
    queryKey: ["rental-provider-owner", provider.admin_user_id],
    queryFn: async () => {
      if (!provider.admin_user_id) return null;
      const { data } = await supabaseDb
        .from("users")
        .select("id, email, name, display_name")
        .eq("id", provider.admin_user_id)
        .maybeSingle();
      return data;
    },
    enabled: !!provider.admin_user_id,
  });

  const { data: managers = [], isLoading } = useQuery({
    queryKey: ["rental-provider-managers", provider.id],
    queryFn: async () => {
      const { data, error } = await supabaseDb
        .from("rental_provider_managers")
        .select("*")
  ...
  const setOwnerMutation = useMutation({
    mutationFn: async () => {
      const { error } = await supabaseDb
        ...
        .eq("id", provider.id);
      if (error) throw error;
      await logAuditEvent(userData!.id, "edit", "rental_provider", provider.id, {
        admin_user_id: ownerForm.user_id.trim() || null,
      });
    },
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: ["rental-provider-owner"] });
  ...
  const addManagerMutation = useMutation({
    mutationFn: async () => {
      ...
      const { data, error } = await supabaseDb
        ...
        .single();
      if (error) throw error;
      await logAuditEvent(userData!.id, "create", "rental_provider_manager", data.id, payload);
    },
    onSuccess: () => {
      toast.success("Manager added");
      qc.invalidateQueries({ queryKey: ["rental-provider-managers", provider.id] });
  ...
  const removeManagerMutation = useMutation({
    mutationFn: async (m: Manager) => {
      const { error } = await supabaseDb
        .from("rental_provider_managers").delete().eq("id", m.id);
      if (error) throw error;
      await logAuditEvent(userData!.id, "delete", "rental_provider_manager", m.id, {});
    },
    onSuccess: () => {
  ...
  const openOwnerDialog = () => {
    setOwnerForm({ user_id: provider.admin_user_id ?? "" });
    setOwnerDialog(true);
  };
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-black tracking-tight">Provider Staff</h2>
        <p className="mt-1 text-sm text-muted-foreground">
          Owner and managers who can access this rental provider
        </p>
      </div>
      ...
      <section className="rounded-2xl border border-border bg-card p-5">
        <div className="mb-4 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-bold uppercase text-xs tracking-wider text-muted-foreground">Owner</h3>
          </div>
        ...
        {owner ? (
          ...
          <p className="text-sm text-muted-foreground italic">
            No owner assigned. Set one to grant provider-level access.
          </p>
        )}
      </section>
      ...
      <section className="rounded-2xl border border-border bg-card p-5">
        ...
        {isLoading ? (
          ...
          <p className="text-sm text-muted-foreground italic">
            No managers yet. Add team members who can manage this provider.
          </p>
        ) : (
          <div className="space-y-2">
            {managers.map((m) => (
      ...
      <section className="rounded-2xl border border-orange-500/20 bg-orange-500/5 p-4 text-sm">
        ...
        <ul className="space-y-1 text-muted-foreground">
          <li>• <span className="text-foreground font-medium">Platform Admins</span> can manage all providers</li>
          <li>• <span className="text-foreground font-medium">Provider Owners</span> can only access their own provider</li>
          <li>• <span className="text-foreground font-medium">Managers</span> can edit vehicles, insurance and extras for their assigned provider</li>
        </ul>
      </section>
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-3">
            <div>
              ...
              />
              <p className="mt-1 text-xs text-muted-foreground">
                Pick the platform user to grant manager access to this provider.
              </p>
            </div>
          </div>
          <DialogFooter>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              this provider's managers? They will lose access immediately.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
