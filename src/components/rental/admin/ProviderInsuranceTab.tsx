...
const EMPTY_FORM = { name: "", price_per_day_dollars: 0, items: "", sort_order: 0, is_active: true };

interface Props {
  providerId: string;
}

export function ProviderInsuranceTab({ providerId }: Props) {
  ...
  const { data: tiers = [], isLoading } = useQuery({
    queryKey: ["admin-rental-insurance-tiers", providerId],
    queryFn: async () => {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["admin-rental-insurance-tiers", providerId] });
  ...
  const openNew = () => { setIsNew(true); setEditTier(null); setForm({ ...EMPTY_FORM, sort_order: tiers.length + 1 }); };
  const openEdit = (t: RentalInsuranceTier) => {
    setIsNew(false); setEditTier(t);
    setForm({ name: t.name, price_per_day_dollars: t.price_per_day_cents / 100, items: (t.items ?? []).join("\n"), sort_order: t.sort_order, is_active: t.is_active });
  };
  const closeDialog = () => { setEditTier(null); setIsNew(false); };
    ...
    <>
      <div className="space-y-4">
        <div className="flex items-center justify-between gap-3">
          <p className="text-sm text-muted-foreground">
            Insurance tiers for this provider. The first tier (lowest sort order) is selected by default.
          </p>
          <Button onClick={openNew}><Plus className="mr-2 h-4 w-4" /> Add Tier</Button>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {tiers.map((t) => (
              <div key={t.id} className={`flex flex-col rounded-2xl border bg-card p-5 ${t.is_active ? "border-border" : "border-border/40 opacity-60"}`}>
                ...
                <div className="mt-4 flex items-center justify-between border-t border-border/60 pt-3">
                  <div className="flex items-center gap-2">
                    <Switch checked={t.is_active} onCheckedChange={(c) => toggleActiveMutation.mutate({ id: t.id, is_active: c })} />
                    <span className="text-xs text-muted-foreground">Visible</span>
                  </div>
                  <div className="flex items-center gap-1">
                    <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => openEdit(t)}><Edit className="h-4 w-4" /></Button>
                    <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive hover:bg-destructive/10 hover:text-destructive" onClick={() => setDeleteTarget(t)}><Trash2 className="h-4 w-4" /></Button>
                  </div>
                </div>
              </div>
            ))}
      ...
      <Dialog open={isNew || !!editTier} onOpenChange={(o) => !o && closeDialog()}>
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-md">
          <DialogHeader><DialogTitle>{isNew ? "Add Insurance Tier" : "Edit Insurance Tier"}</DialogTitle></DialogHeader>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <Input type="number" min={0} step="1" className="pl-6" value={form.price_per_day_dollars}
