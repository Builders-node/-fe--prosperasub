...
export function AccountMenu() {
  const { userData, refreshUserData, isSuperAdmin, logout, isAuthenticated } = useAuth();
  ...
  // Check RBAC admin role (for users who have admin RBAC role but not SUPER_ADMIN JWT role)
  ...
  // Detect Google by email domain — reliable regardless of JWT/session auth_provider inconsistencies
  // (the same account can have auth_provider="email" on one device and "google" on another)
  const isGoogle = email.toLowerCase().includes("@gmail.com") ||
    (userData?.auth_provider === "google") ||
    (userData?.avatar_url?.includes("google") ?? false);
    ...
    <>
      ...
      <DropdownMenu>
        ...
        <AppDropdownContent align="end">
          <AppDropdownProfile title={displayName} subtitle={t("profile.openProfile")} onSelect={() => setOpen(true)} />
          ...
          {isAdminUser && <AppDropdownSeparator />}
          <div className="space-y-space-1">
            {isAdminUser && <AppDropdownItem icon={Shield} title={t("profile.platformAdmin")} subtitle={t("profile.platformAdminDescription")} to="/admin/dashboard" endIcon />}
          </div>
          <AppDropdownSeparator />
          <div className="space-y-space-1">
            <AppDropdownThemeItem />
            <AppDropdownItem icon={LogOut} title={t("profile.logOut")} onSelect={() => void handleLogout()} danger />
          </div>
        </AppDropdownContent>
      </DropdownMenu>

      {/* ── Profile: bottom sheet on mobile, dialog on desktop ── */}
      {(() => {
        // Inner body shared by both Sheet and Dialog (closure over all state)
        const body = (
          <>
            {/* Sub-mode header */}
            {mode !== "view" && (
        ...
        return isMobile ? (
          <Sheet open={open} onOpenChange={setOpen}>
            {/* [&>button]:hidden hides the auto-injected SheetPrimitive close button */}
            <SheetContent
              side="bottom"
              className="!gap-0 !p-0 overflow-hidden rounded-t-[28px] border-0 [&>button]:hidden"
            >
