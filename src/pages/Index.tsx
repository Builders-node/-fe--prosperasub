  ...
  return (
    ...
    >
      ...
      <div className="px-3 py-2.5">
        <p className="truncate text-[13px] font-bold leading-tight" style={{ color: "hsl(var(--yd-text))" }}>
          {name}
        </p>
...
function PlanCard({ id, name, imageUrl, restaurantName, pricePerWeekSats, index }: {
  ...
  return (
    ...
    >
      ...
      <div className="px-3 py-2.5">
        <p className="truncate text-[13px] font-bold" style={{ color: "hsl(var(--yd-text))" }}>{name}</p>
        <p className="mt-0.5 text-[11px]" style={{ color: "hsl(var(--yd-text2))" }}>
...
const Index = () => {
  ...
  return (
    <div style={{ background: "hsl(var(--yd-page))", minHeight: "100dvh" }}>
      {/* Headers */}
      <HomeHeader />
      <DesktopHeader />
      ...
      <div className="mx-auto max-w-[1280px] px-4 pb-24 pt-4 md:px-8 md:pb-16 md:pt-8">
        <div className="space-y-5 md:space-y-10">
          ...
          {/* ── CATEGORY SHORTCUTS (admin only) ─────────────── */}
          {isSuperAdmin && (
            <section>
              <div className="flex gap-5 overflow-x-auto pb-1 scrollbar-hide md:gap-8">
                {CATEGORIES.map((cat) => (
                  <Link key={cat.label} to={cat.href} className="flex shrink-0 flex-col items-center gap-2 transition-transform hover:scale-105">
                    <div className="flex items-center justify-center rounded-full" style={{ width: 62, height: 62, background: "hsl(var(--yd-circle))", fontSize: 26, flexShrink: 0 }}>
                      {cat.emoji}
                    </div>
                    <span className="text-[11px] font-medium lg:text-[13px]" style={{ color: "hsl(var(--yd-text))" }}>{cat.label}</span>
                  </Link>
                ))}
              </div>
            </section>
            ...
            >
              ...
              <div className="relative flex items-end justify-between gap-4">
                <div className="flex-1">
                  <p className="text-[10px] font-bold uppercase tracking-widest lg:text-[11px]" style={{ color: "rgba(255,255,255,0.35)" }}>
                    Premium Service
                  </p>
                  <h2 className="mt-2 text-[22px] font-black leading-tight md:text-[34px]" style={{ letterSpacing: "-0.02em" }}>
                  ...
                  <div className="mt-5 flex flex-wrap gap-3 md:mt-7">
                    <Link to="/cleaning" className="inline-flex h-10 items-center rounded-full bg-white px-6 text-[13px] font-bold text-slate-900 transition hover:bg-white/90 md:h-11 md:px-7 md:text-[14px]">
                      View plans
                    </Link>
                    {isAuthenticated && (
                      <Link to="/cleaning/book" className="inline-flex h-10 items-center rounded-full px-6 text-[13px] font-semibold text-white transition hover:bg-white/10 md:h-11 md:px-7 md:text-[14px]" style={{ border: "1px solid rgba(255,255,255,0.2)" }}>
                        Book session
                      </Link>
                    )}
                  </div>
                ...
                <div className="shrink-0">
                  <span className="block" style={{ fontSize: "clamp(60px, 10vw, 110px)", lineHeight: 1 }}>🧺</span>
                </div>
              </div>
            </div>
          </section>
          ...
          {/* ── POPULAR RESTAURANTS (admin only) ────────────── */}
          {isSuperAdmin && (
            <section>
              <div className="mb-4 flex items-center justify-between">
                <h2 className="text-[18px] font-bold lg:text-[22px]" style={{ color: "hsl(var(--yd-text))", letterSpacing: "-0.02em" }}>
                  Popular
                </h2>
                <Link to="/restaurants" className="inline-flex items-center gap-1 rounded-full px-4 py-1.5 text-[13px] font-semibold transition-colors" style={{ background: "hsl(var(--yd-circle))", color: "hsl(var(--yd-text))" }}>
                  All {restaurantCount > 0 ? restaurantCount : ""} <span style={{ color: "hsl(var(--yd-text2))" }}>›</span>
                </Link>
              </div>
              {restaurantsLoading ? (
                <div className="grid grid-cols-2 gap-3 md:gap-4 lg:grid-cols-4 xl:grid-cols-5">
                  {[0,1,2,3,4].map((i) => <div key={i} className="animate-pulse overflow-hidden rounded-[22px] yd-card" style={{ height: 185 }} />)}
                </div>
              ) : (
                <div className="grid grid-cols-2 gap-3 md:gap-4 lg:grid-cols-3 xl:grid-cols-5">
                  {(restaurants || []).map((restaurant, index) => {
                    const planCount = (restaurant as any).subscription_plans?.filter((p: any) => p.is_active)?.length ?? 0;
                    return (
                      <RestaurantCard key={restaurant.id} id={restaurant.id} name={restaurant.name} logoUrl={restaurant.logo_url} planCount={planCount} index={index} />
                    );
                  })}
                  <Link to="/cleaning" className="group overflow-hidden rounded-[22px] transition-transform duration-150 hover:scale-[1.02]" style={{ background: "linear-gradient(135deg, #1a1a2e, #0f3460)", boxShadow: "0 2px 14px rgba(0,0,0,0.14)", minHeight: 185 }}>
                    <div className="relative flex h-full flex-col justify-between p-4">
                      <div className="flex h-7 w-7 items-center justify-center rounded-full text-sm" style={{ background: "rgba(255,255,255,0.12)" }}>✨</div>
                      <div>
                        <p className="text-[10px] font-bold uppercase tracking-widest" style={{ color: "rgba(255,255,255,0.35)" }}>Service</p>
                        <p className="mt-1 text-[15px] font-black leading-tight text-white">Professional<br />Cleaning</p>
                        <p className="mt-0.5 text-[11px]" style={{ color: "rgba(255,255,255,0.5)" }}>from $79/mo</p>
                        <div className="mt-2.5 inline-flex h-7 items-center rounded-full px-3 text-[11px] font-bold text-white" style={{ background: "rgba(255,255,255,0.15)" }}>Book now</div>
                      </div>
                    </div>
                  </Link>
                </div>
          ...
          {isSuperAdmin && plans && plans.length > 0 && (
            <section>
              <div className="mb-4 flex items-center justify-between">
                <h2
                ...
                >
                  All plans <span style={{ color: "hsl(var(--yd-text2))" }}>›</span>
                </Link>
              </div>
              <div className="grid grid-cols-2 gap-3 md:gap-4 lg:grid-cols-3 xl:grid-cols-5">
          ...
          <section>
            <div
              className="overflow-hidden rounded-[24px] p-6 md:rounded-[28px] md:p-10 yd-card"
              style={{ boxShadow: "var(--yd-card-shadow)" }}
            >
              <div className="mb-6 flex flex-col gap-2 md:mb-8 md:flex-row md:items-end md:justify-between">
                ...
                <p className="text-[13px] leading-relaxed md:max-w-xs md:text-right" style={{ color: "hsl(var(--yd-text2))" }}>
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
                {HOW_IT_WORKS.map((item) => (
                  <div
                    ...
                    style={{ background: "hsl(var(--yd-page))" }}
                  >
                    <div className="mb-4 flex items-center justify-between md:mb-5">
                      <div
                        className="flex h-11 w-11 items-center justify-center rounded-2xl text-[20px] md:h-12 md:w-12 md:text-[22px]"
                        style={{ background: "hsl(var(--yd-circle))" }}
                      >
                        {item.emoji}
                      </div>
                      <span
                        className="text-[32px] font-black md:text-[38px]"
                        style={{ color: "hsl(var(--yd-text) / 0.08)" }}
                      >
                        {item.step}
                      </span>
                    </div>
                    <h3 className="text-[15px] font-bold md:text-[16px]" style={{ color: "hsl(var(--yd-text))" }}>
                      {item.title}
                    </h3>
                    <p className="mt-1.5 text-[12px] leading-relaxed md:mt-2 md:text-[13px]" style={{ color: "hsl(var(--yd-text2))" }}>
                      {item.text}
                    </p>
                  </div>
                ))}
      ...
      <footer className="hidden md:block yd-nav yd-border border-t">
        <div className="mx-auto max-w-[1280px] px-8 py-8">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <Link to="/" className="text-[17px] font-black transition-colors" style={{ color: "hsl(var(--yd-text2))" }}>
              ProsperaSub
            </Link>
            <div className="flex flex-wrap items-center gap-5 text-[13px]" style={{ color: "hsl(var(--yd-text2))" }}>
            ...
            <p className="text-[13px]" style={{ color: "hsl(var(--yd-text2))" }}>{t("footer.copyright")}</p>
          </div>
        </div>
      </footer>
