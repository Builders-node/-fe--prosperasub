 ...
 * slots (a 60-min session with a 30-min buffer steps by 90 minutes), so the
 * label has to survive minutes, not just whole hours.
 */
...
const slotLabel = (from: string, to: string) => `${timeLabel(from)} - ${timeLabel(to)}`;

interface Court { id: string; name: string; type: string; }
/** Slot as generated by the engine (`/booking/availability`). */
interface AvailabilitySlot { from: string; to: string; capacity?: number }
...
const BeachCourts = () => {
  ...
  // Ask the engine which slots exist rather than assuming a whole-hour grid.
  // The provider's schedule drives the step (session duration + buffers), so a
  // 60-min session with a 30-min buffer yields 06:00, 07:30, 09:00 … — a
  // hard-coded hourly list would render slots the engine rejects with
  // `slot_unavailable`, which is exactly the bug this replaces.
  const { data: availability, isLoading: availabilityLoading } = useQuery({
    queryKey: ["beach-engine-availability", resourceId, date] as const,
    enabled: !!resourceId,
    queryFn: async () => {
      ...
      if (error) throw error;
      return (data ?? { slots: [] }) as { slots: AvailabilitySlot[]; reason?: string };
    },
  });
  const slots = availability?.slots ?? [];

  // Bookings keyed by "HH:MM" start so a slot row can find its own booking
  // regardless of whether the grid is hour-aligned.
  ...
  const content = () => {
    ...
    return (
      <>
        ...
        {/* Slots — driven by the engine's availability, not a hard-coded grid. */}
        {availabilityLoading ? (
          <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3">
            {[1, 2, 3, 4, 5, 6].map((i) => <div key={i} className="h-[72px] animate-pulse rounded-xl bg-muted" />)}
          </div>
        ) : slots.length === 0 ? (
          <YdEmptyState
            icon={CalendarDays}
            ...
            subtitle={
              availability?.reason === "resource_not_found"
                ? "This court isn't wired to the booking engine yet."
                : "The venue is closed or fully booked out. Try another date."
            }
          />
        ) : (
          <div className={cn("grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3", bookingsLoading && "opacity-50")}>
            {slots.map((slot) => {
              const b = bookingByStart.get(slot.from);
              const mine = isMine(b);
              return (
                <div key={slot.from}
                  className={cn(
                  ...
                  <div className="min-w-0">
                    <p className="flex items-center gap-1.5 text-sm font-bold text-foreground"><Clock className="h-3.5 w-3.5 text-muted-foreground" />{slotLabel(slot.from, slot.to)}</p>
                  ...
                  {b ? (
                    ...
                    <Button size="sm" className="shrink-0 gap-1 rounded-full bg-primary text-black hover:bg-[hsl(var(--brand-accent-hover))]"
                      disabled={book.isPending} onClick={() => book.mutate(slot.from)}>
